...
class CollisionGridModel(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        self.tensor_embedding_layer = nn.Linear(self.num_TTC*self.num_sector, 
                                                self.embedding_size)
        ...
        self.tensor_embedding_layer_veh = nn.Linear(self.num_TTC_veh*self.num_sector, 
                                                    self.embedding_size) 
    ...
    def getSocialTensor(self, grid, hidden_states, veh_tensor = False):
        ...
        if (veh_tensor == True): 
            social_tensor = Variable(torch.zeros(numNodes, self.num_TTC*self.num_sector,
                                                 self.embedding_size))
    ...
    def getSocialTensor2(self, grid, hidden_states, grid_TTC):
        ...
        grid : Grid masks
        hidden_states : Hidden states of all peds / OR the embedded state of vehicles 
                        /OR the direct action of the neighboring agents
        '''
        # Number of peds
        numNodes = grid.size()[0]
    ...
    def OnePerGridCell(self, grid_current, grid_TTC_current):
        ...
        for col in valid_col_index:
            col_ind = col.item()
            # getting the row indexes of non-zero rows (indexs of neigboring agents located in this specific grid cell) 
    ...
    def forward(self, *args):
        ...
        for framenum,frame in enumerate(input_data):
            ...
            nodes_current = frame[list_of_nodes,:2] # Getting only the x and y of each pedestrian for the input. Leaving the vx and vy
