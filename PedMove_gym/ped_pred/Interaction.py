...
def Time2Conflict(current_position, current_velocity, other_position, other_velocity, Social_dis):
    ...
    if (rel_v_norm2 == 0 or delta < 0):
        ...
        if t2 > 0:
            ...
            # both roots are negative so the agents never get into conflict (they were in conflict preveiously at a time that has now passed)
            t_conflict = -1
...
def approach_dir_sector(current_position, current_velocity, other_position, num_sector):
    ...
    det = current_velocity[0]*rel_position[1] - rel_position[0] * current_velocity[1]  
...
def approach_angle_sector(current_velocity, other_velocity, num_sector):
    ...
    # dot is propotional to cos(theta)
    dot = current_velocity[0]*other_velocity[0] + current_velocity[1] * other_velocity[1] 
    # det (|V_e V_n|) is propotional to sin(theta)
    det = current_velocity[0]*other_velocity[1] - other_velocity[0] * current_velocity[1]
    angle = math.atan2(det, dot) * (180/math.pi)  # the value is between -180 and 180
...
def getInteractionGridMask(frame, frame_other, TTC_min, d_min, num_sector, is_heterogeneous=False, is_occupancy=False, frame_ego=None):
    ...
    for real_frame_index, other_real_frame_index in itertools.product(list_indices, 
                                                                      list_indices_other):
        ...
        t_conflict = Time2Conflict(current_position, current_velocity, 
                                   other_position, other_velocity, d_min)
        ...
        # store_tconflict.append(t_conflict)
        ...
        if is_occupancy:
            ...
            frame_mask[real_frame_index, other_real_frame_index, 
                       time_cell*num_sector+approach_cell] = 1
...
def getSequenceInteractionGridMask(sequence, avail_mask, sequence_veh, avail_mask_veh,
                                   ...
                                   is_heterogeneous=False, is_occupancy=False, 
                                   sequence_ego=None):
    '''
    ...
    for i in range(sl):
        ...
        mask_np, mask_TTC_np = getInteractionGridMask(
                                current_x_seq, current_x_seq_veh, TTC_min, d_min, num_sector,
                                is_heterogeneous, is_occupancy, current_x_seq_ego)
