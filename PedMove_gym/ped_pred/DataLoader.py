...
class DataLoader():
    ...
    def __init__(self, phase='train'):
        ...
        base_dataset = ['PedMove_gym/ped_pred/Data/HBS/hbs.csv']
        ...
        self.data_file_train = os.path.join('PedMove_gym/ped_pred/Data/preprocessed/',
                                             "trajectories_train.cpkl")
        self.data_file_test = os.path.join('PedMove_gym/ped_pred/Data/preprocessed/',
                                            "trajectories_test.cpkl")
        self.data_file_vali = os.path.join('PedMove_gym/ped_pred/Data/preprocessed/', 
                                           "trajectories_validation.cpkl")
    ...
    def frame_preprocess(self, data_dirs, data_file, phase):
        ...
        '''
        # all_frame_data would be a list of list of list of numpy arrays corresponding to
        # each dataset and each target vehicle in that dataset
        # Each numpy array will correspond to a frame and would be of size (numPeds, 6)
        # containing pedID, x, y, vx, vy, timstamp
        all_frame_data_ped = []
       
        # frameList_data would be a list of list of lists corresponding to each dataset and
        # each target vehicle in that dataset
        # Each list would contain the frameIds of all the frames in the dataset where the
        # target agent is present
        frameList_data = []
       
        # numPeds_data would be a list of list of lists corresponding to each dataset and 
        # each target vehicle in that dataset
        # Each list would contain the number of pedestrians in each frame in the dataset
        # where the target vehicle is present
        numPeds_data = []
        ...
        # all_frame_data_ego is a list of list of list of numpy arrays corresponding to 
        # each dataset and each target vehicle in that dataset
        ...
        for directory in data_dirs:
            ...
            column_names = ['frame_id','agent_id','pos_x','pos_y', 'vel_x', 'vel_y',
                             'label', 'timestamp']

            # read train/test/validation file to pandas dataframe and process
            df_orig = pd.read_csv(directory, dtype={'frame_id':'int','agent_id':'int',
                                                     'label':'str'}, usecols=column_names)
            ...
            self.target_ped_ids = np.array(df_ped_orig.drop_duplicates(subset={'agent_id'}, 
                                                                       keep='first', 
                                                                       inplace=False)['agent_id'])
            self.target_veh_ids = np.array(df_veh_orig.drop_duplicates(subset={'agent_id'}, 
                                                                       keep='first', 
                                                                       inplace=False)['agent_id'])
            ...
            data = np.swapaxes(data,0,1) # Now the frame id will propogate thorugh the columns and the frames are all in the first row
                ...
                for ind, frame in enumerate(frameList):
                    ...
                    for ped in pedsList:
                        ...
                        pedsWithPos.append([ped, current_x, current_y,
                                             current_vx, current_vy, timestamp])
                    ...
                    for veh in vehsList:
                        ...
                        vehsWithPos.append([veh, current_x_veh, current_y_veh, 
                                            current_vx_veh, current_vy_veh, timestamp_veh])
    ...
    def convert_proper_array(self, x_seq, pedlist, seq_len):
        ...
        for ind, frame in enumerate(x_seq):
            if len(pedlist[ind]) != 0: # num_pedlist[ind] != 0:
    ...
    def get_len_of_dataset(self):
        # return the number of dataset in this mode
    ...
    def extend_traj2(self, pos_veh, VehsList, lookup_veh, ext_len):
        ...
        ext_pos[:,last_pres_vehs_ind,4] = self.timestamp * torch.ones(ext_len,
                                                                       len(last_pres_vehs_ID))
