...
class Peds(Agent):
    ...
    def get_true_prediction(self, f):
        ...
        current_peds_indx = torch.nonzero(self.mask[f, :]).squeeze()
        if current_peds_indx.dim()==0:
            current_peds_indx = [current_peds_indx.item()]
        else:
            current_peds_indx = current_peds_indx.tolist()
        # edit the prediction data to include only the data of pedestrians that are present in the current frame
        # removing the data of pedestrians that will appear in the future for fair comaprison of truth and inference
        ...
        GT_pred_mask_filtered[:,current_peds_indx] = GT_pred_mask[:,current_peds_indx] # this ensures that peds that disapear before the end of the prediction horizon will have a mask of 0
