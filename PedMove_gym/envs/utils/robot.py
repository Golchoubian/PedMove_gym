from PedMove_gym.envs.utils.agent import Agent
from PedMove_gym.envs.utils.action import ActionXY, ActionRot, ActionAcc, ActionBicycle
import numpy as np
import torch
...
class Robot(Agent):
    ...
    def planned_traj(self):
        ...
        This planned traj will be used for both prediction and plotting (rendering?)
    ...
    def compute_position(self, action):
        ...
        if self.kinematics == 'holonomic':
            ...
            phi = np.clip(self.phi + action.steering_change_rate * self.time_step, -self.max_steering_angle, self.max_steering_angle) # steering angle
    ...
    def compute_velocity(self, action):
        ...
        if self.kinematics == 'holonomic':
            next_vx = action.vx
            next_vy = action.vy
            next_theta = self.calculate_theta(next_vx, next_vy)
        elif self.kinematics == 'unicycle':
            ...
            next_phi = np.clip(self.phi + action.steering_change_rate * self.time_step, -self.max_steering_angle, self.max_steering_angle) # steering angle
    ...
    def human_driver_policy(self, f):
        '''
        This function is only used for running the robot's behaviors
        based on the human driver's actual trajectory in the dataset
        when there is not policy in place for generating the robot's
        action and optimizinig it through a learning process.
        This will generate the ation in an inverse kinematic way
        knowing the trajectory of the human driver in the dataset(
        assuming access to the ground truth). However, due to model inaccuracies
        the generated behaviors this way will only be an approximation of the actual
        human driver's behavior in the dataset
        '''

        current_pos = self.true_pose[f,0,:2]
        current_vel = self.true_pose[f,0,2:4]

        next_pos = self.true_pose[f+1,0,:2]  
        next_vel = self.true_pose[f+1,0,2:4]
        next_v = np.sqrt(next_vel[0]**2 + next_vel[1]**2)

        if self.kinematics == 'holonomic':
            action = ActionXY(next_vel[0], next_vel[1])
        elif self.kinematics == 'unicycle':
            next_theta = self.calculate_theta(next_vel[0], next_vel[1])
            w = next_theta - self.theta
            w = w % (2 * np.pi)
            action = ActionRot(next_v, w)
        else:
            NotImplementedError
